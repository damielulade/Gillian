...
let add_properties_vars properties get_new_scope_id (variables : Variable.ts) =
  let add_lst_vars name to_string lst get_new_scope_id (variables : Variable.ts)
      =
    let list_nodes =
      List.mapi
        (fun index element ->
            ...
            let metadata_node metadata_opt : Variable.t =
              ...
              | Some metadata -> (
                  ...
                  | Expr.ALoc child_loc ->
                      ...
                      Variable.create_node name id ()
                  | _ -> Variable.create_leaf name (to_str Expr.pp metadata) ())
            in
...
let add_variables
  ...
  if is_gil_file then
    ...
    let sorted_locs_with_vals = Legacy_symbolic.sorted_locs_with_vals memory in
